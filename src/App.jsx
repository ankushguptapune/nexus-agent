...
/* ═══════════════════════════════════════════════
   NEXUS AGENT — Secure Team Workspace
   ALL data is encrypted before reaching Firebase.
   Firebase stores only AES-256-GCM ciphertext.
   Even Firebase admins see only random characters.
   ═══════════════════════════════════════════════ */
...
const INIT = {
  ...
  tasks: [], goals: [], schedule: [], followups: [], messages: [],
};
...
function PasswordInput({ value, onChange, placeholder }) {
  ...
  return <div style={{ position:"relative" }}><input type={show?"text":"password"} value={value} onChange={onChange} placeholder={placeholder||"••••••••"} style={{ ...IS, paddingRight:42 }}/><button type="button" onClick={()=>setShow(p=>!p)} style={{ position:"absolute", right:10, top:"50%", transform:"translateY(-50%)", background:"none", border:"none", color:"#5A5E6A", cursor:"pointer" }}><Ic name={show?"eyeOff":"eye"} size={16}/></button></div>;
}

/* ═══ MODAL COMPONENTS (standalone, no <form>) ═══ */
function AddTeamModal({ onClose, onAdd }) {
  const [name,setName]=useState(""); const [role,setRole]=useState(""); const [pw,setPw]=useState("");
  const ok = name.trim() && role.trim() && pw.length>=4;
  return <Modal title="Add Team Member" onClose={onClose}>
    ...
    <FF label="Set Login Password"><PasswordInput value={pw} onChange={e=>setPw(e.target.value)} placeholder="Min 4 characters"/></FF>
    ...
    <div style={{ display:"flex", gap:8, justifyContent:"flex-end" }}><button type="button" onClick={onClose} style={BS}>Cancel</button><button type="button" onClick={()=>{if(ok){onAdd({name:name.trim(),role:role.trim(),password:pw});onClose();}}} style={{...BP,opacity:ok?1:0.4,pointerEvents:ok?"auto":"none"}}>Add Member</button></div>
  </Modal>;
}

function AddTaskModal({ onClose, onAdd, team, prefill="" }) {
  const [title,setTitle]=useState(""); const [desc,setDesc]=useState(""); const [assignee,setAssignee]=useState(prefill); const [pri,setPri]=useState("medium"); const [due,setDue]=useState("");
  const ok=title.trim();
  return <Modal title="Assign Task" onClose={onClose}>
    <FF label="Title"><input value={title} onChange={e=>setTitle(e.target.value)} style={IS} placeholder="e.g. Redesign homepage" autoFocus/></FF>
    <FF label="Description"><textarea value={desc} onChange={e=>setDesc(e.target.value)} rows={2} style={IS}/></FF>
    <FF label="Assign To"><select value={assignee} onChange={e=>setAssignee(e.target.value)} style={SS}><option value="">Unassigned</option>{team.filter(t=>!t.isManager).map(m=><option key={m.id} value={m.name}>{m.name}</option>)}</select></FF>
    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
      <FF label="Priority"><select value={pri} onChange={e=>setPri(e.target.value)} style={SS}><option value="urgent">Urgent</option><option value="high">High</option><option value="medium">Medium</option><option value="low">Low</option></select></FF>
      <FF label="Due Date"><input value={due} onChange={e=>setDue(e.target.value)} type="date" style={IS}/></FF>
    </div>
    <div style={{ display:"flex", gap:8, justifyContent:"flex-end" }}><button type="button" onClick={onClose} style={BS}>Cancel</button><button type="button" onClick={()=>{if(ok){onAdd({title:title.trim(),description:desc,assignee,priority:pri,dueDate:due});onClose();}}} style={{...BP,opacity:ok?1:0.4,pointerEvents:ok?"auto":"none"}}>Create & Notify</button></div>
  </Modal>;
}

function AddFollowupModal({ onClose, onAdd, team }) {
  const m=team.filter(t=>!t.isManager);
  const [title,setTitle]=useState(""); const [person,setPerson]=useState(m[0]?.name||""); const [due,setDue]=useState(""); const [notes,setNotes]=useState("");
  const ok=title.trim()&&due;
  return <Modal title="Add Follow-up" onClose={onClose}>
    ...
    <FF label="Person"><select value={person} onChange={e=>setPerson(e.target.value)} style={SS}>{m.map(x=><option key={x.id} value={x.name}>{x.name}</option>)}</select></FF>
    <FF label="Due Date"><input value={due} onChange={e=>setDue(e.target.value)} type="date" style={IS}/></FF>
    <FF label="Notes"><textarea value={notes} onChange={e=>setNotes(e.target.value)} rows={2} style={IS}/></FF>
    <div style={{ display:"flex", gap:8, justifyContent:"flex-end" }}><button type="button" onClick={onClose} style={BS}>Cancel</button><button type="button" onClick={()=>{if(ok){onAdd({title:title.trim(),person,dueDate:due,notes});onClose();}}} style={{...BP,opacity:ok?1:0.4,pointerEvents:ok?"auto":"none"}}>Add & Notify</button></div>
  </Modal>;
}

function AddGoalModal({ onClose, onAdd }) {
  const [title,setTitle]=useState(""); const [desc,setDesc]=useState(""); const [type,setType]=useState("short_term"); const [dl,setDl]=useState(""); const [ms,setMs]=useState("");
  const ok=title.trim();
  return <Modal title="New Goal" onClose={onClose}>
    <FF label="Title"><input value={title} onChange={e=>setTitle(e.target.value)} style={IS} autoFocus/></FF>
    <FF label="Description"><textarea value={desc} onChange={e=>setDesc(e.target.value)} rows={2} style={IS}/></FF>
    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}><FF label="Type"><select value={type} onChange={e=>setType(e.target.value)} style={SS}><option value="short_term">Short-term</option><option value="long_term">Long-term</option></select></FF><FF label="Deadline"><input value={dl} onChange={e=>setDl(e.target.value)} type="date" style={IS}/></FF></div>
    <FF label="Milestones (one per line)"><textarea value={ms} onChange={e=>setMs(e.target.value)} rows={3} style={IS} placeholder={"Research\nWireframes\nMVP"}/></FF>
    <div style={{ display:"flex", gap:8, justifyContent:"flex-end" }}><button type="button" onClick={onClose} style={BS}>Cancel</button><button type="button" onClick={()=>{if(ok){onAdd({title:title.trim(),description:desc,type,deadline:dl,milestones:ms.split("\n").filter(Boolean).map(t=>({text:t.trim(),done:false}))});onClose();}}} style={{...BP,opacity:ok?1:0.4,pointerEvents:ok?"auto":"none"}}>Create Goal</button></div>
  </Modal>;
}

function AddScheduleModal({ onClose, onAdd }) {
  const [title,setTitle]=useState(""); const [date,setDate]=useState(today()); const [time,setTime]=useState(""); const [desc,setDesc]=useState("");
  const ok=title.trim()&&time;
  return <Modal title="Schedule Event" onClose={onClose}>
    ...
    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}><FF label="Date"><input value={date} onChange={e=>setDate(e.target.value)} type="date" style={IS}/></FF><FF label="Time"><input value={time} onChange={e=>setTime(e.target.value)} type="time" style={IS}/></FF></div>
    <FF label="Details"><textarea value={desc} onChange={e=>setDesc(e.target.value)} rows={2} style={IS}/></FF>
    <div style={{ display:"flex", gap:8, justifyContent:"flex-end" }}><button type="button" onClick={onClose} style={BS}>Cancel</button><button type="button" onClick={()=>{if(ok){onAdd({title:title.trim(),date,time,description:desc});onClose();}}} style={{...BP,opacity:ok?1:0.4,pointerEvents:ok?"auto":"none"}}>Add Event</button></div>
  </Modal>;
}

function ChangePwModal({ user, onClose, onSave }) {
  const [cur,setCur]=useState(""); const [np,setNp]=useState(""); const [cf,setCf]=useState(""); const [err,setErr]=useState(""); const [done,setDone]=useState(false);
  const submit=()=>{ if(cur!==user.password){setErr("Current password is wrong");return;} if(np.length<4){setErr("Min 4 characters");return;} if(np!==cf){setErr("Passwords don't match");return;} onSave(np);setDone(true); };
  ...
  return <Modal title="Change Password" onClose={onClose} width={420}>
    <FF label="Current Password"><PasswordInput value={cur} onChange={e=>{setCur(e.target.value);setErr("");}}/></FF>
    <FF label="New Password"><PasswordInput value={np} onChange={e=>{setNp(e.target.value);setErr("");}}/></FF>
    <FF label="Confirm"><PasswordInput value={cf} onChange={e=>{setCf(e.target.value);setErr("");}}/></FF>
...
function ResetPwModal({ member, onClose, onSave }) {
  const [pw,setPw]=useState(""); const [done,setDone]=useState(false);
  if(done) return <Modal title="Done" onClose={onClose} width={380}><div style={{textAlign:"center",padding:10}}><div style={{width:48,height:48,borderRadius:"50%",background:"#00C9A720",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 14px",color:"#00C9A7"}}><Ic name="check" size={24}/></div><p style={{color:"#E0E0E5",fontWeight:600}}>Password reset for {member.name}!</p><button type="button" onClick={onClose} style={{...BP,marginTop:10}}>Done</button></div></Modal>;
  return <Modal title={`Reset — ${member.name}`} onClose={onClose} width={420}>
...
export default function App() {
  ...
  // Save to Firebase (encrypted automatically by firebase.js)
  useEffect(()=>{ if(!loading&&user) saveShared(D); },[D,loading]);
  ...
  const addMsg=(msg)=>u("messages",p=>[...p,{id:uid(),timestamp:new Date().toISOString(),readBy:[],...msg}]);
  const markRead=(ids)=>u("messages",p=>p.map(m=>ids.includes(m.id)?{...m,readBy:[...new Set([...(m.readBy||[]),myName])]}:m));
  const changePw=(mid,pw)=>u("team",p=>p.map(t=>t.id===mid?{...t,password:pw}:t));

  const addTask=(t)=>{
    u("tasks",p=>[...p,{id:uid(),createdAt:today(),status:"todo",...t}]);
    if(t.assignee&&t.assignee!=="Manager") addMsg({from:"Manager",to:t.assignee,type:"task",body:`📋 New task: "${t.title}"\nPriority: ${PCOL[t.priority]?.l||"Medium"}\nDue: ${t.dueDate||"No deadline"}`});
  };
  const updateTask=(id,up)=>{
    u("tasks",p=>p.map(t=>t.id===id?{...t,...up}:t));
    if(up.status&&!isManager){const tk=tasks.find(t=>t.id===id);if(tk)addMsg({from:myName,to:"Manager",type:"status",body:`✅ "${tk.title}" → ${STAT[up.status]}`});}
  };
  ...
  const addFollowup=(f)=>{
    ...
    if(f.person&&f.person!=="Manager") addMsg({from:"Manager",to:f.person,type:"followup",body:`🔔 Follow-up: "${f.title}"\nDue: ${f.dueDate}${f.notes?"\n"+f.notes:""}`});
  };
  ...
  const sendMsg=async(to,body)=>{if(body.trim())addMsg({from:myName,to,type:"message",body});};
    ...
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#0D0F14",fontFamily:"'DM Sans',sans-serif",padding:20}}>
      ...
      <div style={{width:"100%",maxWidth:400,animation:"fadeIn .5s ease"}}>
        ...
        <div style={{background:"#12141A",borderRadius:14,padding:24,border:"1px solid #1E2028"}}>
          ...
          <FF label="Password"><PasswordInput value={loginPw} onChange={e=>{setLoginPw(e.target.value);setLoginErr("");}}/></FF>
        ...
        <p style={{color:"#3A3D45",fontSize:11,textAlign:"center",marginTop:14}}>Manager default: admin123</p>
      </div>
    </div>
  );
    ...
    <div style={{display:"flex",height:"100vh",fontFamily:"'DM Sans',sans-serif",background:"#0D0F14",color:"#E8E8ED",overflow:"hidden"}}>
      ...
      <div style={{width:sideOpen?220:60,background:"#12141A",borderRight:"1px solid #1E2028",display:"flex",flexDirection:"column",transition:"width .2s",flexShrink:0,overflow:"hidden"}}>
        ...
        <nav style={{flex:1,padding:"10px 6px",display:"flex",flexDirection:"column",gap:2}}>
          {navItems.map(i=><button type="button" key={i.id} onClick={()=>setView(i.id)} style={{display:"flex",alignItems:"center",gap:11,padding:sideOpen?"9px 13px":"9px",background:view===i.id?"#00C9A710":"transparent",border:"none",borderRadius:9,color:view===i.id?"#00C9A7":"#5A5E6A",cursor:"pointer",fontSize:13,fontWeight:600,fontFamily:"inherit",transition:"all .15s",textAlign:"left",justifyContent:sideOpen?"flex-start":"center",whiteSpace:"nowrap"}}><Ic name={i.icon} size={17}/>{sideOpen&&i.label}{sideOpen&&i.id==="comms"&&myUnread>0&&<span style={{marginLeft:"auto",background:"#E01E5A",color:"#fff",fontSize:10,fontWeight:700,padding:"1px 6px",borderRadius:8}}>{myUnread}</span>}</button>)}
        </nav>
      ...
      <div style={{flex:1,overflow:"auto",padding:"24px 28px"}}>
        ...
        {view==="dashboard"&&(<div style={{animation:"fadeIn .3s"}}>
          ...
          <p style={{color:"#5A5E6A",marginTop:3,fontSize:13,display:"flex",alignItems:"center",gap:6}}><Ic name="encrypted" size={13}/> All data encrypted · {new Date().toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric"})}</p>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:12,marginTop:20}}>
            {[{l:isManager?"Active Tasks":"My Tasks",v:activeTasks,c:"#00C9A7",go:"tasks"},{l:"Completed",v:completedTasks,c:"#45B7D1",go:"tasks"},{l:"Unread",v:myUnread,c:"#E01E5A",go:"comms"},{l:"Goals",v:`${goalProg}%`,c:"#BB8FCE",go:"goals"},{l:"Events",v:todaySch.length,c:"#FFD60A",go:"schedule"},...(isManager?[{l:"Follow-ups",v:pendingFU,c:"#FF9500",go:"followups"}]:[])].map((s,i)=><div key={i} onClick={()=>setView(s.go)} style={{background:"#12141A",borderRadius:12,padding:"16px 18px",border:"1px solid #1E2028",cursor:"pointer"}}><div style={{fontSize:11,color:"#5A5E6A",fontWeight:700,textTransform:"uppercase"}}>{s.l}</div><div style={{fontSize:26,fontWeight:800,color:s.c,marginTop:4}}>{s.v}</div></div>)}
          </div>
        ...
        {view==="comms"&&(<div style={{animation:"fadeIn .3s",display:"flex",gap:14,height:"calc(100vh - 48px)"}}>
          <div style={{width:240,flexShrink:0}}>
            ...
            {chatContacts.map(c=>{const unread=getUnreadFrom(c.name);const last=getConvo(c.name).slice(-1)[0];return <button type="button" key={c.id} onClick={()=>{setActiveChat(c.name);const ids=messages.filter(m=>m.from===c.name&&m.to===myName&&!m.readBy?.includes(myName)).map(m=>m.id);if(ids.length)markRead(ids);}} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",background:activeChat===c.name?"#00C9A708":"#12141A",border:`1px solid ${activeChat===c.name?"#00C9A730":"#1E2028"}`,borderRadius:10,cursor:"pointer",textAlign:"left",fontFamily:"inherit",marginBottom:6,width:"100%"}}><Avatar name={c.name} size={34} mgr={c.isManager}/><div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:600,color:"#E0E0E5"}}>{c.name}</div><div style={{fontSize:11,color:"#5A5E6A",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{last?.body?.slice(0,30)||c.role}</div></div>{unread>0&&<span style={{background:"#E01E5A",color:"#fff",fontSize:10,fontWeight:700,padding:"1px 6px",borderRadius:8}}>{unread}</span>}</button>;})}
          </div>
          <div style={{flex:1,display:"flex",flexDirection:"column",background:"#12141A",borderRadius:12,border:"1px solid #1E2028"}}>
            {activeChat?<><div style={{padding:"14px 18px",borderBottom:"1px solid #1E2028",display:"flex",alignItems:"center",gap:10}}><Avatar name={activeChat} size={30} mgr={activeChat==="Manager"}/><div style={{flex:1}}><div style={{fontSize:14,fontWeight:700}}>{activeChat}</div></div><div style={{display:"flex",alignItems:"center",gap:4,color:"#00C9A7",fontSize:11,fontWeight:600}}><Ic name="encrypted" size={13}/> Encrypted</div></div>
              <div style={{flex:1,overflow:"auto",padding:18}}>{getConvo(activeChat).map(m=><div key={m.id} style={{display:"flex",justifyContent:m.from===myName?"flex-end":"flex-start",marginBottom:10}}><div style={{maxWidth:"75%",padding:"10px 14px",borderRadius:12,background:m.from===myName?"linear-gradient(135deg,#00C9A7,#00B4D8)":"#1E2028",color:m.from===myName?"#fff":"#E0E0E5"}}><div style={{fontSize:13,lineHeight:1.5,whiteSpace:"pre-wrap"}}>{m.body}</div><div style={{fontSize:9,color:m.from===myName?"#ffffff80":"#4A4D55",marginTop:4,textAlign:"right"}}>{timeAgo(m.timestamp)}</div></div></div>)}<div ref={msgRef}/>{!getConvo(activeChat).length&&<p style={{color:"#4A4D55",textAlign:"center",padding:40,fontSize:13}}>No messages yet</p>}</div>
              <div style={{padding:"12px 18px",borderTop:"1px solid #1E2028",display:"flex",gap:8}}><input value={msgInput} onChange={e=>setMsgInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&msgInput.trim()){sendMsg(activeChat,msgInput);setMsgInput("");}}} placeholder={`Message ${activeChat}...`} style={{...IS,flex:1}}/><button type="button" onClick={()=>{if(msgInput.trim()){sendMsg(activeChat,msgInput);setMsgInput("");}}} style={BP}><Ic name="send" size={16}/></button></div>
            </>:<div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:8,color:"#4A4D55"}}><Ic name="encrypted" size={32}/><span>Select a conversation</span><span style={{fontSize:11}}>All data is AES-256 encrypted</span></div>}
          </div>
        </div>)}

        {view==="team"&&isManager&&(<div style={{animation:"fadeIn .3s"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:22}}><div><h1 style={{fontSize:24,fontWeight:800,margin:0}}>Team</h1><p style={{color:"#5A5E6A",margin:"3px 0 0",fontSize:13}}>{team.filter(t=>!t.isManager).length} members</p></div><button type="button" onClick={()=>setModal("addTeam")} style={BP}><Ic name="plus" size={14}/> Add Member</button></div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:12}}>
            {team.filter(t=>!t.isManager).map(m=>{const mt=tasks.filter(t=>t.assignee===m.name);return <div key={m.id} style={{background:"#12141A",borderRadius:12,padding:18,border:"1px solid #1E2028"}}>
        ...
        {view==="tasks"&&(<div style={{animation:"fadeIn .3s"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:22}}><div><h1 style={{fontSize:24,fontWeight:800,margin:0}}>{isManager?"All Tasks":"My Tasks"}</h1><p style={{color:"#5A5E6A",margin:"3px 0 0",fontSize:13}}>{activeTasks} active · {completedTasks} done</p></div>{isManager&&<button type="button" onClick={()=>setModal("addTask")} style={BP}><Ic name="plus" size={14}/> New Task</button>}</div>
          {["todo","in_progress","review","blocked","done"].map(st=>{const gr=myTasks.filter(t=>t.status===st);if(!gr.length)return null;return <div key={st} style={{marginBottom:20}}><div style={{fontSize:11,fontWeight:700,color:"#5A5E6A",textTransform:"uppercase",marginBottom:8}}>{STAT[st]} <span style={{background:"#1A1D23",padding:"1px 6px",borderRadius:6,fontSize:10}}>{gr.length}</span></div>{gr.map(t=><div key={t.id} style={{background:"#12141A",borderRadius:10,padding:"12px 16px",border:"1px solid #1E2028",marginBottom:6,display:"flex",alignItems:"center",gap:12}}><div style={{flex:1}}><div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}><span style={{fontWeight:600,fontSize:13,color:t.status==="done"?"#5A5E6A":"#E8E8ED",textDecoration:t.status==="done"?"line-through":"none"}}>{t.title}</span><PBadge p={t.priority}/>{t.dueDate&&t.dueDate<today()&&t.status!=="done"&&<Badge color="#FF3B30">Overdue</Badge>}</div><div style={{fontSize:11,color:"#5A5E6A",marginTop:3}}>{isManager&&t.assignee&&<>→ <strong style={{color:"#8E8E93"}}>{t.assignee}</strong> · </>}{t.dueDate&&`Due ${t.dueDate}`}</div></div><select value={t.status} onChange={e=>updateTask(t.id,{status:e.target.value})} style={{...SS,padding:"5px 7px",fontSize:10,width:"auto",minWidth:90}}>{Object.entries(STAT).map(([k,v])=><option key={k} value={k}>{v}</option>)}</select>{isManager&&<button type="button" onClick={()=>deleteTask(t.id)} style={{background:"none",border:"none",color:"#4A4D55",cursor:"pointer"}}><Ic name="trash" size={14}/></button>}</div>)}</div>;})}
        ...
        {view==="followups"&&isManager&&(<div style={{animation:"fadeIn .3s"}}>
          ...
          {followups.filter(f=>!f.completed).map(f=><div key={f.id} style={{background:"#12141A",borderRadius:10,padding:"12px 16px",border:"1px solid #1E2028",marginBottom:6,display:"flex",alignItems:"center",gap:12}}><button type="button" onClick={()=>toggleFollowup(f.id)} style={{width:20,height:20,borderRadius:5,border:"2px solid #3A3D45",background:"none",cursor:"pointer",flexShrink:0}}/><div style={{flex:1}}><div style={{fontWeight:600,fontSize:13}}>{f.title}</div><div style={{fontSize:11,color:"#5A5E6A"}}>{f.person} · Due {f.dueDate}</div></div><button type="button" onClick={()=>deleteFollowup(f.id)} style={{background:"none",border:"none",color:"#4A4D55",cursor:"pointer"}}><Ic name="trash" size={14}/></button></div>)}
        ...
        {view==="goals"&&(<div style={{animation:"fadeIn .3s"}}>
          ...
          {["short_term","long_term"].map(type=>{const gr=goals.filter(g=>g.type===type);if(!gr.length)return null;return <div key={type} style={{marginBottom:24}}><div style={{fontSize:11,fontWeight:700,color:"#5A5E6A",textTransform:"uppercase",marginBottom:10}}>{type==="short_term"?"Short-term":"Long-term"}</div>{gr.map(g=><div key={g.id} style={{background:"#12141A",borderRadius:12,padding:"16px 18px",border:"1px solid #1E2028",marginBottom:8}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><div style={{fontWeight:700,fontSize:14}}>{g.title}</div>{isManager&&<button type="button" onClick={()=>deleteGoal(g.id)} style={{background:"none",border:"none",color:"#4A4D55",cursor:"pointer"}}><Ic name="trash" size={14}/></button>}</div><div style={{display:"flex",alignItems:"center",gap:10}}><div style={{flex:1}}><Bar value={g.progress} color={type==="short_term"?"#00C9A7":"#BB8FCE"}/></div><span style={{fontSize:13,fontWeight:700,color:type==="short_term"?"#00C9A7":"#BB8FCE"}}>{g.progress}%</span></div>{isManager&&<input type="range" min="0" max="100" value={g.progress} onChange={e=>updateGoal(g.id,{progress:parseInt(e.target.value)})} style={{width:"100%",marginTop:6,accentColor:type==="short_term"?"#00C9A7":"#BB8FCE"}}/>}{g.milestones?.length>0&&<div style={{marginTop:10}}>{g.milestones.map((m,i)=><div key={i} style={{display:"flex",alignItems:"center",gap:7,fontSize:12,color:"#5A5E6A",padding:"2px 0",cursor:isManager?"pointer":"default"}} onClick={()=>{if(!isManager)return;const up=[...g.milestones];up[i]={...m,done:!m.done};updateGoal(g.id,{milestones:up,progress:Math.round(up.filter(x=>x.done).length/up.length*100)});}}><span style={{color:m.done?"#00C9A7":"#3A3D45"}}>{m.done?"✓":"○"}</span><span style={{textDecoration:m.done?"line-through":"none"}}>{m.text}</span></div>)}</div>}</div>)}</div>;})}
        ...
        {view==="ai"&&(<div style={{animation:"fadeIn .3s",display:"flex",flexDirection:"column",height:"calc(100vh - 48px)"}}>
          ...
          {!aiMsgs.length&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:14}}>{(isManager?["Focus priorities?","Team workload","Deadlines at risk?","Plan my week"]:["My priorities?","Help with task","What's overdue?","Update for manager"]).map(q=><button type="button" key={q} onClick={()=>setAiIn(q)} style={{padding:"9px 12px",background:"#12141A",border:"1px solid #1E2028",borderRadius:9,color:"#6B6F7B",fontSize:12,cursor:"pointer",fontFamily:"inherit",textAlign:"left"}}>{q}</button>)}</div>}
          <div style={{flex:1,overflowY:"auto",marginBottom:12}}>{aiMsgs.map((m,i)=><div key={i} style={{display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start",marginBottom:10}}><div style={{maxWidth:"75%",padding:"11px 15px",borderRadius:12,background:m.role==="user"?"linear-gradient(135deg,#00C9A7,#00B4D8)":"#12141A",color:m.role==="user"?"#fff":"#E0E0E5",border:m.role==="ai"?"1px solid #1E2028":"none",fontSize:13,lineHeight:1.5,whiteSpace:"pre-wrap"}}>{m.text}</div></div>)}{aiLoad&&<div style={{display:"flex"}}><div style={{padding:"11px 15px",borderRadius:12,background:"#12141A",border:"1px solid #1E2028",display:"flex",gap:5}}>{[0,1,2].map(i=><div key={i} style={{width:7,height:7,borderRadius:"50%",background:"#00C9A7",animation:`pulse 1.2s ease-in-out ${i*.2}s infinite`}}/>)}</div></div>}<div ref={chatRef}/></div>
          <div style={{display:"flex",gap:8,paddingBottom:6}}><input value={aiIn} onChange={e=>setAiIn(e.target.value)} onKeyDown={e=>e.key==="Enter"&&sendAI()} placeholder="Ask AI..." style={{...IS,flex:1}}/><button type="button" onClick={sendAI} disabled={aiLoad} style={{...BP,opacity:aiLoad?.5:1}}><Ic name="send" size={15}/></button></div>
        </div>)}
      </div>
      ...
      {(modal==="addTask"||modal?.type==="addTask")&&<AddTaskModal onClose={()=>setModal(null)} onAdd={addTask} team={team} prefill={modal?.assignee||""}/>}
